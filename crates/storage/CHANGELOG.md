...
All notable changes to `pipa-storage` (renamed from `pipa-data`) are documented in this file.
...
- **Renamed from `pipa-data` to `pipa-storage`**, and the crate directory from `crates/data`
  to `crates/storage` — no functional change, just the name.

## [0.3.0] - 2026-09-25

### Removed

- Iceberg catalog/query access (`catalog.rs`, `query.rs`; `IcebergCatalogConfig`,
  `QueryError`, `QueryService`) — moved to the new `pipa-iceberg` crate, which depends on
  `pipa-data` for `ObjectStoreConfig` rather than the other way around. `pipa-data` no longer
  depends on `iceberg`/`iceberg-datafusion`/`datafusion` at all.

## [0.2.1] - 2026-09-24

### Fixed

- `ProjectService::register`/`update` now reject a name already held by another project
  (case-insensitive, trimmed) via a new `ProjectError::DuplicateName` variant, instead of
  silently allowing two projects to share a name.
